...
async def update_exchange_rates():
    async with httpx.AsyncClient() as client:
        response = await client.get(
            ...
            "app_id=8c1573bcf1d042f59a4e05178205ce4f"
        )
        data = response.json()
...
async def last_update():
    db = SessionLocal()
    latest_currency = (
        db.query(Currency).order_by(Currency.updated_at.desc())
        .first()
    )
    db.close()
    if latest_currency:
...
async def convert(request: ConversionRequest):
    db = SessionLocal()
    source = (
        db.query(Currency).filter(Currency.code == request.source_currency)
        .first()
    )
    target = (
        db.query(Currency).filter(Currency.code == request.target_currency)
        .first()
    )
    db.close()
